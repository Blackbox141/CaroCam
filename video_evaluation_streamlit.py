...
def search_backwards_for_mate_or_matt(cap, start_frame, max_frames_fix, piece_model, state_info, frame_interval_search):
    ...
    while current_frame>=0:
        if count_attempts> max_frames_fix:
            st.info("Suche nach weiteren Zügen abgebrochen: max_frames_fix erreicht.")
            break
        count_attempts+=1
...
def main():
    ...
    if not video_file:
        st.info("Lade ein Video hoch, um zu Analyse zu starten.")
        return
    ...
    if start_button:
        ...
        with st.expander("Hinter den Kulissen"):
            ...
            if not ret:
                ...
                st.subheader("Alle erkannten Objekte (Eckpunkte, Figuren, Uhr inkl. Vektoren/ROI)")
        ...
        if not found_start:
            st.warning("Keine automatische Grundstellung gefunden. Manuell wählen:")
        ...
        def process_normal_state(frame, frame_number, cap, params, models, stinfo):
            ...
            # Falls sich was an der Uhr geändert hat (Clock-Wechsel)
            if pturn and pturn!="hold" and pturn!= stinfo['previous_player_turn']:
                ...
                if cfenX== prev_fen:
                    ...
                    if mvX and mvX in board.legal_moves:
                        ...
                        # 2) Direkte Matt-Prüfung
                        if board.is_checkmate():
                            # Expander mit "Matt!"
                            halfm= stinfo['halfmove_count']
                            if color=='w':
                                move_label= f"Zug {(halfm+1)//2}a: {mvX} (Matt!)"
                            else:
                                move_label= f"Zug {(halfm+1)//2}b: {mvX} (Matt!)"

                            with st.expander(move_label):
                                # Minimaldarstellung oder das volle 3-Spalten-Layout
                                col1, col2, col3 = st.columns([1.0,1.0,1.0])

                                with col1:
                                    st.image(frame, caption=f"Frame {frame_number}", channels="BGR")

                                fen_matt = f"{cfenX} {('w' if color=='b' else 'b')} KQkq - 0 1"
                                bsvg_matt = create_chessboard_svg(fen_matt, last_move=mvX)
                                with col2:
                                    st.markdown("**Gespielter Mattzug**")
                                    st.components.v1.html(bsvg_matt, height=400)
                        ...
                        with st.expander(move_label):
                            ...
                            with col2:
                                ...
                                # Stockfish-Bewertung
                                bestM, evaluation, mate= None,"None",None
                ...
                if current_fenZ== prev_fen:
                    ...
                    if mvZ and mvZ in board.legal_moves:
                        ...
                        if board.is_checkmate():
                            halfm= stinfo['halfmove_count']
                            if color=='w':
                                move_label= f"Zug {(halfm+1)//2}a: {mvZ} (Matt!)"
                            else:
                                move_label= f"Zug {(halfm+1)//2}b: {mvZ} (Matt!)"
                            ...
                            with st.expander(move_label):
                                ...
                                fen_matt= f"{current_fenZ} {('w' if color=='b' else 'b')} KQkq - 0 1"
                                bsvg_matt= create_chessboard_svg(fen_matt, last_move=mvZ)
                                with col2:
                                    st.markdown("**Gespielter Mattzug**")
                                    st.components.v1.html(bsvg_matt, height=400)
                        ...
                        with st.expander(move_label):
                            ...
                            with col3:
                                if bestS:
                                    st.markdown("**Empfohlener nächster Zug**: " + bestS)
        ...
        def process_fixing_state(frame, frame_number, cap, params, models, stinfo):
            ...
            if mvFix and mvFix in board.legal_moves:
                ...
                if board.is_checkmate():
                    halfm= stinfo['halfmove_count']
                    if color=='w':
                        move_label= f"Zug {(halfm+1)//2}a: {mvFix} (Matt!)"
                    else:
                        move_label= f"Zug {(halfm+1)//2}b: {mvFix} (Matt!)"
                    ...
                    with st.expander(move_label):
                        ...
                        fen_matt= f"{fenFix} {('w' if color=='b' else 'b')} KQkq - 0 1"
                        bsvg_matt= create_chessboard_svg(fen_matt, last_move= mvFix)
                        with col2:
                            st.markdown("**Gespielter Mattzug**")
                            st.components.v1.html(bsvg_matt, height=400)
                ...
                with st.expander(move_label):
                    ...
                    with col3:
                        if bestF:
                            st.markdown("**Empfohlener nächster Zug**: " + bestF)
        ...
        frame_counter_placeholder.text("100% | Analyse abgeschlossen")

        if state_info['global_board'].is_game_over():
            if state_info['global_board'].is_checkmate():
                winner= "Weiss" if state_info['color']=="w" else "Schwarz"
                st.success(f"Glückwunsch, {winner} hat gewonnen!")
            else:
                st.info("Partie ist zu Ende (z.B. Patt oder Abbruch).")
        elif state_info['state']=="finished":
            winner = "Weiss" if state_info['color'] == "w" else "Schwarz"
            st.success(f"Glückwunsch, {winner} hat gewonnen!")
        else:
            st.info("Keine Matt-Situation gefunden, Analyse beendet.")
