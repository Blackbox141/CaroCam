    ...
    if uploaded_file is not None:
        ...
        if st.button("Zwischenschritte anzeigen"):
            ...
            for box in piece_results.boxes:
                x1, y1, x2, y2 = box.xyxy.cpu().numpy()[0]
                cls = int(box.cls.cpu().numpy()[0])
                label = piece_results.names[cls]
                conf = box.conf.cpu().numpy()[0]
                rect = plt.Rectangle((x1, y1), x2 - x1, y2 - y1, linewidth=2, edgecolor='green', facecolor='none')
                ax1.add_patch(rect)
                ax1.text(x1, y1 - 10, f'{label} {conf:.2f}', color='white', fontsize=12,
                         bbox=dict(facecolor='green', alpha=0.5))
            ax1.axis('off')
            st.pyplot(fig1)
            ...
            for label, point in detected_points.items():
                ax2.plot(point[0], point[1], 'ro')  # Rote Punkte für Eckpunkte
                ax2.text(point[0], point[1], label, fontsize=12, color='white',
                         bbox=dict(facecolor='red', alpha=0.5))
            ax2.axis('off')
            st.pyplot(fig2)
