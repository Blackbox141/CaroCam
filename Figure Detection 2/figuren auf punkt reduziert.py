...
model = YOLO('runs/detect/yolov8-chess28/weights/best.pt')
...
# Funktion, um die Ergebnisse mit Bounding Boxes zu plotten und die Mittelpunkte anzuzeigen
def plot_results_with_custom_font(result, img, font_size=8, line_thickness=1):
    ...
    for box in result.boxes:
        ...
        # Berechnung des Mittelpunkts der unteren Kante
        mid_x = x1 + width / 2
        mid_y = y1 + height * 0.75  # Punkt in der Mitte der unteren Hälfte

        # Zeichnen des Punkts (mittig an der unteren Seite)
        ax.plot(mid_x, mid_y, 'o', color='red', markersize=10)  # roter Punkt als Marker
...
def predict_from_path(image_path):
    ...
    results = model.predict(img, conf=0.1, iou=0.3, imgsz=1400)
    result = results[0]
    ...
    output_path = "annotated_output_with_midpoints.png"
